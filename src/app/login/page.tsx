...
export default function LoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(json.error ?? t("auth.loginFailed"));
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-6 max-w-md w-full mx-auto pt-2 v-fade-in">
        ...
        <p className="text-[var(--vie-ink-soft)] mt-1">{t("auth.signInSubtitle")}</p>
        ...
        <form onSubmit={submit} className="mt-7 v-card space-y-4">
          ...
          <div>
            <label className="v-label">{t("auth.password")}</label>
          ...
          <button type="submit" disabled={loading} className="v-btn v-btn-primary w-full">
            {loading ? <Loader2 className="animate-spin" size={16} /> : <>{t("auth.signIn")} <ArrowRight size={16} /></>}
          </button>
          <div className="flex items-center justify-between text-sm">
            <Link href="/forgot-password" className="text-[var(--vie-ink-muted)] hover:text-[var(--vie-ink)]">{t("auth.forgot")}</Link>
            <Link href="/signup" className="text-[var(--vie-red)] font-semibold">{t("auth.signUp")}</Link>
          </div>
        </form>

        <div className="mt-8">
          <div className="flex items-center gap-2 mb-3 px-1">
            <span className="v-pill v-pill-brand text-[10px]">{t("auth.demoOnly")}</span>
            <span className="text-xs text-[var(--vie-ink-muted)]">{t("auth.demoSubtitle")}</span>
          </div>
          <div className="grid gap-2">
            {demoUsers.map((u) => (
              ...
              >
                ...
                <span className="text-xs text-[var(--vie-ink-muted)]">{t("auth.installer")}</span>
              </button>
            ))}
            <button
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="font-semibold text-sm truncate">{t("auth.adminAccount")}</div>
                <div className="text-xs text-[var(--vie-ink-muted)] truncate">{t("auth.adminDescription")}</div>
              </div>
              <span className="text-xs text-[var(--vie-ink-muted)]">{t("auth.adminLabel")}</span>
            </button>
          </div>
          <p className="text-[11px] text-[var(--vie-ink-muted)] mt-3 px-1">
            {t("auth.allPasswords")}: <code className="px-1 py-0.5 bg-[var(--vie-line)] rounded">demo1234</code> · {t("auth.adminPassword")}: <code className="px-1 py-0.5 bg-[var(--vie-line)] rounded">admin1234</code>
          </p>
        </div>
      </main>
    </div>
